...
const passwordSchema = z.object({
    email: z.string().min(1, { message: "Email address is required." }).email({ message: "Please enter a valid email address (e.g., you@example.com)." }),
        ...
        <div className="min-h-screen flex items-center justify-center px-4 bg-zinc-50 dark:bg-zinc-950 font-sans transition-colors duration-300">
            ...
            <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 rounded-3xl shadow-xl shadow-zinc-100/50 dark:shadow-none p-8 relative overflow-hidden">
                ...
                {!magicLinkSent && !otpSent && (
                    <div className="mb-6 flex gap-1 rounded-xl bg-zinc-100 dark:bg-zinc-950 p-1 border border-zinc-200/30 dark:border-zinc-800/30">
                        <button
                            ...
                            className={`flex-1 rounded-lg py-2 text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer ${method === "password" ? "bg-white dark:bg-zinc-900 text-indigo-600 dark:text-indigo-400 shadow-sm" : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"}`}
                        >
                            Password
                        </button>
                        <button
                            ...
                            className={`flex-1 rounded-lg py-2 text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer ${method === "magic_link" ? "bg-white dark:bg-zinc-900 text-indigo-600 dark:text-indigo-400 shadow-sm" : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"}`}
                        >
                            Magic Link
                        </button>
                        <button
                            ...
                            className={`flex-1 rounded-lg py-2 text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer ${method === "otp" ? "bg-white dark:bg-zinc-900 text-indigo-600 dark:text-indigo-400 shadow-sm" : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"}`}
                        >
                            OTP
                        </button>
                    </div>
                ...
                {method === "password" && (
                    ...
                    ${passwordForm.formState.errors.email ? "border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-100 dark:focus:ring-red-955/30" : "border-zinc-200 dark:border-zinc-800 focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/30"}`}
                                />
                            </div>
                            {passwordForm.formState.errors.email && <p className="text-xs font-medium text-red-500 mt-1 pl-1">{passwordForm.formState.errors.email.message}</p>}
                        </div>
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                <Link href="/forgot-password" className="text-xs font-semibold text-zinc-500 hover:text-zinc-955 dark:hover:text-white transition-colors">
                                    Forgot password?
                                </Link>
                            </div>
                            <div className="relative group">
                    ...
                    ${passwordForm.formState.errors.password ? "border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-100 dark:focus:ring-red-955/30" : "border-zinc-200 dark:border-zinc-800 focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/30"}`}
                                />
                                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
                                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                </button>
                            </div>
                            {passwordForm.formState.errors.password && <p className="text-xs font-medium text-red-500 mt-1 pl-1">{passwordForm.formState.errors.password.message}</p>}
                        </div>

                        {/* Submit button */}
                        <button type="submit" disabled={isSubmitting} className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-md">
                            {isSubmitting ? (
                                <>
                    ...
                    <div>
                        {!magicLinkSent ? (
                            <form onSubmit={emailForm.handleSubmit(onMagicLinkRequest)} className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <input {...emailForm.register("email")} type="email" placeholder="you@example.com" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white text-sm font-medium transition-all duration-200 outline-none focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/30" />
                                    </div>
                                    {emailForm.formState.errors.email && <p className="text-xs font-medium text-red-500 mt-1 pl-1">{emailForm.formState.errors.email.message}</p>}
                                </div>

                                <button type="submit" disabled={isSubmitting} className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shadow-md">
                                    {isSubmitting ? (
                                        <Loader2 className="w-4 h-4 animate-spin" />
                                    ) : (
                                        <>
                            ...
                            <div className="py-4 text-center space-y-4">
                                ...
                                <button onClick={() => setMagicLinkSent(false)} className="text-xs font-semibold text-zinc-500 hover:text-zinc-950 dark:hover:text-white hover:underline transition-colors">
                    ...
                    <div>
                        {!otpSent ? (
                            ...
                            <form onSubmit={otpForm.handleSubmit(onOtpVerify)} className="space-y-4">
                                ...
                                {otpForm.formState.errors.otp && <p className="text-center text-xs font-medium text-red-500">{otpForm.formState.errors.otp.message}</p>}
                                ...
                                <button type="button" onClick={() => setOtpSent(false)} className="w-full text-center text-xs font-semibold text-zinc-500 hover:text-zinc-950 dark:hover:text-white hover:underline transition-colors pt-2">
                ...
                {!magicLinkSent && !otpSent && (
                    <>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {/* GOOGLE BUTTON */}
                            <button type="button" onClick={() => handleSocialLogin("google")} className="flex items-center justify-center gap-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-white py-2.5 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-100 cursor-pointer shadow-sm">
                            ...
                            {/* GITHUB BUTTON */}
                            <button type="button" onClick={() => handleSocialLogin("github")} className="flex items-center justify-center gap-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-white py-2.5 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-100 cursor-pointer shadow-sm">
                ...
                <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800/80 text-center">
                    <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400">
                        ...
                        <Link href="/register" className="text-zinc-950 dark:text-white hover:underline font-bold">
                            Sign Up Free
                        </Link>
                    </p>
                </div>
